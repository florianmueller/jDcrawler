    ...
    for entry in prefiltered_values:
        ...
        for show in shows:
            ...
            if 'title' in show_info and 'screen_size' in show_info:
                ...
                #to get info about link (either 'episode' oder 'movie')
                info_type = show_info['type']
                #print('type:' +info_type+'    show_info screen:' +screen_size+'    show:'+show+'    title:'+title)
                #print(raw_title)
                #print('  ')
                if show.lower() in title.lower() and quality == screen_size and info_type == 'movie' and hosterShort in raw_title and not download_exists2(identifier=title):
                    print('type:            ' +info_type+'    show_info screen:' +screen_size+'    show:'+show+'    title:'+title)
                    
                    #deep link filter hoster shorten URL reste (dirty)
                    filteredLinkSlice = filter_link(link, hosterName)
                    #length = len(filteredLinkSlice)
                    ...
                    persist_download2(identifier=title)
